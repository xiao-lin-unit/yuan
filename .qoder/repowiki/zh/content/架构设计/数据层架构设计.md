...
本财务应用程序采用双模式数据库架构设计，支持Capacitor SQLite（移动端原生）和SQL.js（Web端）两种数据库实现。该架构通过统一的DatabaseManager单例模式管理数据库连接，实现了跨平台的数据持久化解决方案。系统采用Vue 3 + Pinia的状态管理模式，结合CRUD抽象封装、事务处理和错误处理机制，构建了一个完整的财务数据管理系统。
...
subgraph "状态管理层"
PS[Pinia Store<br/>账户管理]
CS[CategoryService<br/>分类服务]
end
subgraph "UI层"
AM[AccountManagement<br/>账户管理]
DV[DatabaseViewer<br/>数据库查看]
AS[AssetManagement<br/>资产管理]
end
subgraph "外部依赖"
CAP[Capacitor SQLite]
SQLJS[SQL.js]
PINIA[Pinia状态管理]
end
DM --> DB
...
DatabaseManager --> AccountStore : "提供数据访问"
CategoryService --> AccountStore : "数据关联"
```
...
该架构为财务应用程序提供了坚实的数据基础，能够满足复杂财务数据管理的需求，为用户提供稳定可靠的服务体验。