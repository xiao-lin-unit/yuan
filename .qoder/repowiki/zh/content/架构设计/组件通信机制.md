...
- [src\components\mobile\sandbox\FinancialSandbox.vue](file://src\components\mobile\sandbox\FinancialSandbox.vue)
- [src\components\mobile\sandbox\SandboxSimulationPage.vue](file://src\components\mobile\sandbox\SandboxSimulationPage.vue)
- [src\components\mobile\sandbox\SandboxHistory.vue](file://src\components\mobile\sandbox\SandboxHistory.vue)
- [src\components\mobile\sandbox\SandboxResultDetail.vue](file://src\components\mobile\sandbox\SandboxResultDetail.vue)
...
## 更新摘要
**变更内容**
- 新增完整的沙盘模块化架构，包括财务推演场景、参数配置、计算引擎
- 扩展事件发射机制，支持复杂的参数传递和路由导航
- 更新组件映射系统，支持沙盘相关组件的动态加载
- 改进导航参数处理，支持带参导航和参数验证
...
- **新增** 沙盘模块化架构：财务推演场景、参数配置、计算引擎的完整实现
- **新增** 复杂事件通信：支持带参导航、参数验证、事件链路管理
- 示例与最佳实践：帮助开发者理解复杂交互模式

## 项目结构
项目采用"容器-展示"分层与"按功能域组织"的目录结构，核心导航与布局由 App.vue 控制，内容区通过动态组件加载具体业务页面。新增的沙盘模块提供了完整的财务推演功能。
...
SimPage["SandboxSimulationPage.vue"]
History["SandboxHistory.vue"]
ResultDetail["SandboxResultDetail.vue"]
Service["sandboxService.ts"]
end
App --> Header
App --> Content
App --> Footer
...
Sandbox --> SimPage
Sandbox --> History
SimPage --> ResultDetail
Service --> Sandbox
Service --> SimPage
Service --> History
Service --> ResultDetail
```

**图表来源**
- [src\App.vue:1-273](file://src\App.vue#L1-L273)
...
- [src\components\mobile\sandbox\FinancialSandbox.vue:1-121](file://src\components\mobile\sandbox\FinancialSandbox.vue#L1-L121)
- [src\components\mobile\sandbox\SandboxSimulationPage.vue:1-154](file://src\components\mobile\sandbox\SandboxSimulationPage.vue#L1-L154)
- [src\components\mobile\sandbox\SandboxHistory.vue:1-145](file://src\components\mobile\sandbox\SandboxHistory.vue#L1-L145)
- [src\components\mobile\sandbox\SandboxResultDetail.vue:1-323](file://src\components\mobile\sandbox\SandboxResultDetail.vue#L1-L323)
- [src\services\sandbox\sandboxService.ts:1-722](file://src\services\sandbox\sandboxService.ts#L1-L722)
...
- **新增** FinancialSandbox.vue：沙盘主界面，提供多种财务推演场景的选择，向上抛出导航事件。
- **新增** SandboxSimulationPage.vue：沙盘推演页面，动态生成参数表单，执行推演计算并导航到结果详情。
- **新增** SandboxHistory.vue：沙盘历史记录页面，展示推演历史，支持查看详情和删除操作。
- **新增** SandboxResultDetail.vue：沙盘结果详情页面，展示推演结果的可视化图表和详细分析。
- **新增** sandboxService.ts：沙盘服务层，提供场景定义、参数配置、计算引擎和数据持久化。
...
## 架构总览
应用采用"事件驱动 + 动态组件 + 全局状态 + 沙盘模块"的组合模式：
...
- **新增** 沙盘模块：提供完整的财务推演功能，包括场景定义、参数配置、计算引擎和结果展示。
...
participant SimPage as "SandboxSimulationPage.vue"
User->>Footer : 点击底部导航
Footer-->>App : 触发 navigate(key)
App->>App : 更新 activeMenu/navParams
App->>Content : 传入 currentComponent/props
Content->>Sandbox : 动态渲染组件 + 透传 props
Sandbox-->>SimPage : emit('navigate', {key : 'sandboxSimulation', params : {sceneType}})
SimPage-->>App : emit('navigate', {key : 'sandboxResultDetail', params : {historyId}})
App->>App : 处理导航/参数传递
```

**图表来源**
- [src\components\common\AppFooter.vue:29-31](file://src\components\common\AppFooter.vue#L29-L31)
- [src\App.vue:194-201](file://src\App.vue#L194-L201)
- [src\components\common\AppContent.vue:18-21](file://src\components\common\AppContent.vue#L18-L21)
- [src\components\mobile\sandbox\FinancialSandbox.vue:49-51](file://src\components\mobile\sandbox\FinancialSandbox.vue#L49-L51)
- [src\components\mobile\sandbox\SandboxSimulationPage.vue:100-112](file://src\components\mobile\sandbox\SandboxSimulationPage.vue#L100-L112)
...
- **新增** 沙盘组件通过 emit 传递复杂的导航参数，包括场景类型和历史记录ID。
- **新增** SandboxSimulationPage 通过 emit('navigate', {key:'sandboxResultDetail', params:{historyId}}) 实现带参导航。

```mermaid
flowchart TD
A["App.vue<br/>维护 activeMenu/navParams"] --> B["AppContent.vue<br/>:is 动态组件 + v-bind props"]
B --> C["业务组件<br/>ExpensePage/AccountManagement"]
B --> D["沙盘组件<br/>FinancialSandbox/SandboxSimulationPage"]
C -- "$emit('navigate')" --> B
D -- "$emit('navigate', {key, params})" --> B
B -- "$emit('dateChange')" --> A
D --> E["sandboxService.ts<br/>计算引擎 + 数据持久化"]
```
...
- [src\components\mobile\sandbox\FinancialSandbox.vue:49-51](file://src\components\mobile\sandbox\FinancialSandbox.vue#L49-L51)
- [src\components\mobile\sandbox\SandboxSimulationPage.vue:100-112](file://src\components\mobile\sandbox\SandboxSimulationPage.vue#L100-L112)
- [src\services\sandbox\sandboxService.ts:278-696](file://src\services\sandbox\sandboxService.ts#L278-L696)
...
- **新增** 沙盘组件通过 emit('navigate') 事件实现组件间的参数传递和导航控制。
...
- **新增** 沙盘组件通过 emit('navigate') 事件实现跨层级的导航控制，支持带参导航。
...
- **新增** 复杂事件参数：沙盘模块的导航事件支持复杂的参数对象，包括 key 和 params 字段。
...
- **新增** 沙盘数据传递：SandboxSimulationPage 通过 emit('navigate', {key:'sandboxResultDetail', params:{historyId}}) 实现推演结果的导航传递。
- **新增** 参数验证：main.ts 中的 appNavigateTo 函数支持新旧两种导航参数格式，提供参数验证和转换。
...
D -- "emit('navigate', {key, params})" --> B
B -- "emit('dateChange')" --> A
E["Pinia Store<br/>useAccountStore"] --> C
F["CreditCardItem.vue<br/>emit('click', cardId)"] --> C
G["FundItem.vue<br/>emit('click', fundId)"] --> C
H["sandboxService.ts<br/>计算引擎 + 数据持久化"] --> D
I["main.ts<br/>appNavigateTo 参数验证"] --> A
```
...
- [src\components\mobile\sandbox\SandboxSimulationPage.vue:100-112](file://src\components\mobile\sandbox\SandboxSimulationPage.vue#L100-L112)
- [src\services\sandbox\sandboxService.ts:278-696](file://src\services\sandbox\sandboxService.ts#L278-L696)
- [src\main.ts:95-110](file://src\main.ts#L95-L110)
...
### 应用级导航系统
- 组件映射：App.vue 维护 componentMap，将路由键映射到具体业务组件，包括新增的沙盘组件。
- 动态组件加载：AppContent 通过 :is 动态渲染组件，并透传 props。
- 导航参数：App.vue 维护 navParams，支持带参导航（如 fundId、sceneType、historyId）。
- **更新** 统一导航处理：AccountManagement.vue 中的 navigateToAccountDetail 方法统一处理所有账户详情导航请求。
- **新增** 沙盘导航映射：main.ts 中新增 sandbox、sandboxSimulation、sandboxHistory、sandboxResultDetail 组件映射。
- **新增** 参数化组件属性：appComponentProps 函数支持沙盘相关组件的参数传递，包括 sceneType 和 historyId。
...
C --> E["沙盘组件<br/>FinancialSandbox/SandboxSimulationPage/SandboxHistory/SandboxResultDetail"]
F["AppFooter/SideMenu/AppHeader"] --> A
G["CreditCardItem/FundItem<br/>emit('click', accountId)"] --> H["AccountManagement.vue<br/>navigateToAccountDetail"]
H --> A
I["Sandbox组件<br/>emit('navigate', {key,params})"] --> A
J["main.ts<br/>新增沙盘组件映射"] --> B
K["appComponentProps<br/>支持沙盘参数"] --> A
```

**图表来源**
- [src\App.vue:75-113](file://src\App.vue#L75-L113)
- [src\components\common\AppContent.vue:3-8](file://src\components\common\AppContent.vue#L3-L8)
- [src\components\common\AppFooter.vue:29-31](file://src\components\common\AppFooter.vue#L29-L31)
...
- [src\components\mobile\sandbox\FinancialSandbox.vue:49-51](file://src\components\mobile\sandbox\FinancialSandbox.vue#L49-L51)
- [src\components\mobile\sandbox\SandboxSimulationPage.vue:100-112](file://src\components\mobile\sandbox\SandboxSimulationPage.vue#L100-L112)
- [src\main.ts:85-92](file://src\main.ts#L85-L92)
- [src\main.ts:171-179](file://src\main.ts#L171-L179)
...
- **新增** 沙盘组件：FinancialSandbox.vue 在 onMounted 中加载场景数据，SandboxSimulationPage.vue 动态生成参数表单。
- **新增** 计算引擎：sandboxService.ts 提供复杂的财务推演计算，包括多种场景的数学模型。
- **新增** 数据持久化：SandboxSimulationPage.vue 通过 saveSandboxSimulation 保存推演结果到数据库。
...
- [src\components\common\PageTemplate.vue:34-37](file://src\components\common\PageTemplate.vue#L34-L37)
- [src\components\mobile\sandbox\FinancialSandbox.vue:86-94](file://src\components\mobile\sandbox\FinancialSandbox.vue#L86-L94)
- [src\components\mobile\sandbox\SandboxSimulationPage.vue:86-94](file://src\components\mobile\sandbox\SandboxSimulationPage.vue#L86-L94)
- [src\services\sandbox\sandboxService.ts:278-696](file://src\services\sandbox\sandboxService.ts#L278-L696)
...
- **新增** 沙盘模块化：通过 sandboxService.ts 将计算逻辑与UI组件分离，提高代码复用性。
- **新增** 参数化导航：通过 appNavigateTo 函数统一处理导航参数，支持新旧两种格式。
...
### 组件通信示例与最佳实践
- 示例路径
  - 顶部菜单切换侧边菜单：[src\components\common\AppHeader.vue:45-47](file://src\components\common\AppHeader.vue#L45-L47) → [src\App.vue:204-211](file://src\App.vue#L204-L211)
  - 底部导航切换页面：[src\components\common\AppFooter.vue:29-31](file://src\components\common\AppFooter.vue#L29-L31) → [src\App.vue:194-201](file://src\App.vue#L194-L201)
  ...
  - **新增** 沙盘导航：[src\components\mobile\sandbox\FinancialSandbox.vue:49-51](file://src\components\mobile\sandbox\FinancialSandbox.vue#L49-L51) → [src\components\mobile\sandbox\SandboxSimulationPage.vue:100-112](file://src\components\mobile\sandbox\SandboxSimulationPage.vue#L100-L112) → [src\App.vue:194-201](file://src\App.vue#L194-L201)
  - **新增** 参数化导航：[src\main.ts:95-110](file://src\main.ts#L95-L110) → [src\main.ts:171-179](file://src\main.ts#L171-L179)
...
- 最佳实践
  ...
  - **新增** 沙盘模块最佳实践：使用 sandboxService.ts 集中管理计算逻辑，组件只负责UI展示和参数收集。
  - **新增** 参数验证：在 main.ts 中使用 appNavigateTo 函数统一处理导航参数，支持新旧格式兼容。
...
## 依赖关系分析
- 应用入口：main.ts 创建应用并注册 Pinia、ElementPlus，新增沙盘组件映射。
...
- **新增** 沙盘服务层：sandboxService.ts 提供完整的财务推演服务，包括场景定义、参数配置、计算引擎和数据持久化。
- **新增** 参数处理层：main.ts 提供导航参数的统一处理和验证。

```mermaid
graph LR
Main["main.ts<br/>新增沙盘组件映射"] --> App["App.vue"]
App --> Header["AppHeader.vue"]
App --> Footer["AppFooter.vue"]
App --> Content["AppContent.vue"]
App --> Menu["SideMenu.vue"]
Content --> Pages["业务组件"]
Content --> SandboxPages["沙盘组件"]
...
FundItem --> AccMgr
AccMgr --> App
SandboxPages --> Service["sandboxService.ts<br/>计算引擎 + 数据持久化"]
Service --> DB
Main --> Service
```
...
- **新增** 沙盘计算优化：sandboxService.ts 使用高效的数学计算算法，支持大规模数据处理。
- **新增** 参数化导航：main.ts 中的 appNavigateTo 函数提供参数验证和转换，减少错误处理开销。
...
- 键盘事件异常：检查 Capacitor 键盘插件初始化与事件监听。
  - 参考：[src\App.vue:213-239](file://src\App.vue#L213-L239)
- 导航参数丢失：确认 navigateTo 的参数传递与 navParams 存储。
  - 参考：[src\App.vue:194-201](file://src\App.vue#L194-L201)
...
- **新增** 沙盘推演失败：检查 sandboxService.ts 的计算逻辑和数据库连接。
  - 参考：[src\services\sandbox\sandboxService.ts:278-696](file://src\services\sandbox\sandboxService.ts#L278-L696)
- **新增** 导航参数格式错误：检查 main.ts 中的 appNavigateTo 函数参数处理。
  - 参考：[src\main.ts:95-110](file://src\main.ts#L95-L110)
...
**章节来源**
- [src\App.vue:213-239](file://src\App.vue#L213-L239)
- [src\App.vue:194-201](file://src\App.vue#L194-L201)
- [src\components\mobile\expense\TopNav.vue:73-83](file://src\components\mobile\expense\TopNav.vue#L73-L83)
- [src\components\mobile\expense\ExpensePage.vue:46-49](file://src\components\mobile\expense\ExpensePage.vue#L46-L49)
- [src\components\common\FloatingActionMenu.vue:44-59](file://src\components\common\FloatingActionMenu.vue#L44-L59)
...
## 结论
本项目通过"事件驱动 + 动态组件 + 全局状态 + 沙盘模块"的组合，实现了清晰的组件通信与导航体系。App.vue 作为根容器承担了统一的导航与状态管理职责，业务组件专注于自身功能，配合 Pinia 与数据库层，形成了高内聚、低耦合的架构。

**更新亮点**：新增的沙盘模块提供了完整的财务推演功能，包括多种推演场景、参数配置、计算引擎和结果展示。通过 sandboxService.ts 将复杂的计算逻辑与UI组件分离，提高了代码的可维护性和复用性。CreditCardItem.vue 和 FundItem.vue 新增统一的点击事件处理器，实现了标准化的导航交互模式。AccountManagement.vue 通过统一的 navigateToAccountDetail 方法，确保所有账户详情导航请求都经过一致的处理流程。main.ts 中的 appNavigateTo 函数支持新旧两种导航参数格式，提供了更好的兼容性。
...
- **新增** 沙盘模块测试：建议为 sandboxService.ts 添加单元测试，验证各种推演场景的计算准确性。
- **新增** 参数验证测试：建议为 main.ts 中的 appNavigateTo 函数添加测试，确保参数格式转换的正确性。
- **新增** 沙盘组件测试：建议为 FinancialSandbox.vue、SandboxSimulationPage.vue、SandboxHistory.vue、SandboxResultDetail.vue 添加组件测试，验证导航和数据流的正确性。