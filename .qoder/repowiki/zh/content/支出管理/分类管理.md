...
```mermaid
sequenceDiagram
participant UI as "界面层<br/>AddExpensePage.vue"
...
UI->>CS : 调用 getCategories("expense")
CS->>DB : 查询自定义分类
DB-->>CS : 返回自定义分类列表
CS->>DEF : 读取默认支出分类
DEF-->>CS : 返回默认分类列表
...
**图表来源**
- [AddExpensePage.vue:202-228](file://src/components/mobile/expense/AddExpensePage.vue#L202-L228)
...
- 支出分类（expenseCategories）
  - 覆盖日常消费的主要类别：餐饮、零食、服装、交通、旅行、孩子、宠物、通讯、烟酒、学习、日用品、住房、美妆、医疗、红包、汽车/加油、娱乐、礼金、数码电器、运动、其他、水电煤等
  - 业务含义：帮助用户快速归类日常支出，便于统计与预算控制
- 收入分类（incomeCategories）
  - 覆盖主要收入来源：工资、奖金、投资收益、兼职、礼金、其他
...
- [AddExpensePage.vue:197-213](file://src/components/mobile/expense/AddExpensePage.vue#L197-L213)
...
**图表来源**
- [AddExpensePage.vue:420-470](file://src/components/mobile/expense/AddExpensePage.vue#L420-L470)
...
- 组件依赖
  ...
  - AddExpensePage 依赖 CategoryService 与 CategoryItem
- 服务依赖
...
AEP["AddExpensePage.vue"] --> CS["categoryService.ts"]
CS --> DB["database/index.js"]
CS --> CAT
...
**图表来源**
- [CategoryItem.vue:9-21](file://src/components/mobile/expense/CategoryItem.vue#L9-L21)
- [AddExpensePage.vue:113-117](file://src/components/mobile/expense/AddExpensePage.vue#L113-L117)
...
## 结论
本分类管理方案通过清晰的组件与服务分层、完善的默认分类与自定义分类合并策略、以及跨平台的数据库适配，实现了稳定可靠的分类管理能力。建议在后续迭代中引入分类层级、批量重分类、分类统计等增强功能，以进一步提升用户体验与数据治理水平。
