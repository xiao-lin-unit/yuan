...
## 更新摘要
**变更内容**
- 财务仪表板组件目录结构重组：从 `mobile/financial/FinancialDashboard.vue` 重命名为 `mobile/dashboard/FinancialDashboard.vue`
- 更新所有 import 路径以反映新的组件位置
- 修复 App.vue 中的错误导入路径，移除不必要的 `/index.ts`
- 财务仪表板图表优化：容器高度从260px减少到200px（23%空间节省）
- 轴标签字体优化：x轴和y轴标签字体从10px增加到11px提升可读性
- 时间范围选择器样式优化：新增样式类以去除阴影和边框
- 图表网格配置优化：顶部边距12%防止标签截断
...
**图表来源**
- [FinancialDashboard.vue:141-210](file://src/components/mobile/dashboard/FinancialDashboard.vue#L141-L210)
- [FinancialDashboard.vue:93-138](file://src/components/mobile/dashboard/FinancialDashboard.vue#L93-L138)
- [FinancialDashboard.vue:542-628](file://src/components/mobile/dashboard/FinancialDashboard.vue#L542-L628)
...
**图表来源**
- [FinancialDashboard.vue:329-358](file://src/components/mobile/dashboard/FinancialDashboard.vue#L329-L358)

**章节来源**
- [FinancialDashboard.vue:329-385](file://src/components/mobile/dashboard/FinancialDashboard.vue#L329-L385)
...
**章节来源**
- [FinancialDashboard.vue:392-539](file://src/components/mobile/dashboard/FinancialDashboard.vue#L392-L539)
...
**图表来源**
- [FinancialDashboard.vue:541-628](file://src/components/mobile/dashboard/FinancialDashboard.vue#L541-L628)
...
**章节来源**
- [FinancialDashboard.vue:213-327](file://src/components/mobile/dashboard/FinancialDashboard.vue#L213-L327)
...
### 图表容器高度优化
**更新** 图表容器高度从260px减少到200px，实现了23%的空间节省，提升了移动端的布局效率。

```css
.chart-container {
  width: 100%;
  height: 200px; /* 从260px优化为200px */
}
```

### 轴标签字体优化
**更新** x轴和y轴标签字体从10px增加到11px，显著提升了图表的可读性和用户体验。

```javascript
xAxis: {
  type: 'category',
  data: history.map((h: any) => h.label),
  axisLabel: { fontSize: 11 } /* 从10px优化为11px */
},
yAxis: {
  type: 'value',
  axisLabel: {
    ...
    fontSize: 11 /* 从10px优化为11px */
  }
}
```

### 时间范围选择器样式优化
**更新** 新增时间范围选择器样式类，专门用于去除阴影和边框，保持界面简洁统一。
...
### 图表网格配置优化
**更新** 图表网格配置优化，顶部边距设置为12%，有效防止了标签被截断的问题。
...
这些优化措施共同提升了财务仪表板的视觉效果和用户体验，确保图表在各种设备上都能完美显示。

**章节来源**
- [FinancialDashboard.vue:541-590](file://src/components/mobile/dashboard/FinancialDashboard.vue#L541-L590)
- [FinancialDashboard.vue:785-805](file://src/components/mobile/dashboard/FinancialDashboard.vue#L785-L805)
...
**图表来源**
- [FinancialDashboard.vue:67-71](file://src/components/mobile/dashboard/FinancialDashboard.vue#L67-L71)
...
6. **图表性能优化**：通过高度优化、字体优化和样式优化提升视觉效果
...
### 目录结构变更说明
**更新** 财务仪表板组件目录结构已重组：
- **原路径**：`src/components/mobile/financial/FinancialDashboard.vue`
- **新路径**：`src/components/mobile/dashboard/FinancialDashboard.vue`
- **影响范围**：所有引用该组件的文件都需要更新导入路径
- **修复内容**：App.vue中的错误导入路径已修正，移除了不必要的 `/index.ts`

**章节来源**
- [main.ts:22](file://src/main.ts#L22)
- [App.vue:40](file://src/App.vue#L40)