...
- [src/services/knowledge/knowledgeService.ts](file://src/services/knowledge/knowledgeService.ts)
...
- [src/components/mobile/knowledge/KnowledgeHome.vue](file://src/components/mobile/knowledge/KnowledgeHome.vue)
- [src/components/mobile/knowledge/KnowledgeArticle.vue](file://src/components/mobile/knowledge/KnowledgeArticle.vue)
- [src/components/mobile/knowledge/KnowledgeProfile.vue](file://src/components/mobile/knowledge/KnowledgeProfile.vue)
...
## 更新摘要
**变更内容**
- 新增知识收藏表(knowledge_favorite)和阅读历史表(knowledge_read_history)，支持用户个性化知识管理功能
- 知识管理功能涵盖文章分类、收藏、阅读历史记录、最近阅读等功能
- 数据库初始化流程包含新的知识管理表结构
- 新增相关索引以优化知识管理查询性能
...
**更新** 本次更新反映了数据库架构的重大变更，从简单的integer-ended标志迁移到TEXT状态字段系统，新增了更灵活的状态管理机制，支持'开启'、'暂停'、'结束'等状态枚举值，使系统能够更好地管理复杂的财务状态变化。同时，新增了知识管理功能，包括知识收藏表和阅读历史表，支持用户个性化知识管理。
...
SQLite --> Categories
SQLite --> KnowledgeFavorite
SQLite --> KnowledgeReadHistory
```

**图表来源**
- [src/database/index.js:1-1064](file://src/database/index.js#L1-L1064)
...
### 知识收藏表 (KnowledgeFavorite)

**新增** 知识收藏表用于存储用户收藏的文章。

| 字段名 | 数据类型 | 约束条件 | 描述 |
|--------|----------|----------|------|
| article_id | TEXT | PRIMARY KEY | 文章ID（外键） |
| create_time | TEXT | NOT NULL | 收藏时间 |
| article_id | TEXT | FOREIGN KEY(articles.id) | 外键约束 |

### 阅读历史表 (KnowledgeReadHistory)
...
|--------|----------|----------|------|
| article_id | TEXT | PRIMARY KEY | 文章ID（外键） |
| read_time | TEXT | NOT NULL | 阅读时间 |
| is_read | INTEGER | DEFAULT 1 | 是否已读标记 |
| article_id | TEXT | FOREIGN KEY(articles.id) | 外键约束 |

**章节来源**
- [src/database/index.js:823-841](file://src/database/index.js#L823-L841)
- [src/database/index.js:842-866](file://src/database/index.js#L842-L866)
- [src/services/knowledge/knowledgeService.ts:100-175](file://src/services/knowledge/knowledgeService.ts#L100-L175)
...
**新增** 系统新增了完整的知识管理功能，包括文章分类、收藏、阅读历史等特性。

#### 核心功能
- 文章分类管理（基础管理、风险管控、投资入门、重大决策、陷阱规避、书籍分享）
- 文章内容管理（支持本地内容和外部文件内容）
- 用户个性化功能（收藏、阅读历史、最近阅读）
- 知识内容的Markdown渲染和展示
...
```mermaid
flowchart TD
Start([开始知识管理]) --> LoadArticles["加载文章数据"]
LoadArticles --> LoadCategories["加载分类数据"]
LoadCategories --> UserActions["用户操作"]
UserActions --> Favorite{"收藏操作"}
UserActions --> ReadHistory{"阅读历史"}
UserActions --> RecentRead{"最近阅读"}
Favorite --> AddFavorite["添加收藏"]
AddFavorite --> QueryFavorites["查询收藏"]
ReadHistory --> RecordRead["记录阅读"]
RecordRead --> QueryHistory["查询历史"]
QueryHistory --> ClearHistory["清空历史"]
RecentRead --> GetRecent["获取最近阅读"]
GetRecent --> DisplayRecent["显示最近阅读"]
```
...
#### 知识内容管理
系统支持两种内容加载方式：
1. **本地内容**：直接从JSON配置中读取内容数组
2. **外部文件**：通过Vite的import.meta.glob预加载Markdown文件
...
**章节来源**
- [src/services/knowledge/knowledgeService.ts:1-175](file://src/services/knowledge/knowledgeService.ts#L1-L175)
- [src/components/mobile/knowledge/KnowledgeHome.vue:1-326](file://src/components/mobile/knowledge/KnowledgeHome.vue#L1-L326)
- [src/components/mobile/knowledge/KnowledgeArticle.vue:1-548](file://src/components/mobile/knowledge/KnowledgeArticle.vue#L1-L548)
- [src/components/mobile/knowledge/KnowledgeProfile.vue:1-148](file://src/components/mobile/knowledge/KnowledgeProfile.vue#L1-L148)
...
KnowledgeFavorite[KnowledgeFavorite表]
KnowledgeReadHistory[KnowledgeReadHistory表]
end
...
KnowledgeHome --> KnowledgeService
KnowledgeArticle --> KnowledgeService
KnowledgeProfile --> KnowledgeService
AccountStore --> DatabaseManager
CategoryService --> DatabaseManager
GoalService --> DatabaseManager
KnowledgeService --> DatabaseManager
...
DatabaseAdapter --> KnowledgeFavorite
DatabaseAdapter --> KnowledgeReadHistory
```
...
- [src/components/mobile/knowledge/KnowledgeHome.vue:88-144](file://src/components/mobile/knowledge/KnowledgeHome.vue#L88-L144)
- [src/components/mobile/knowledge/KnowledgeArticle.vue:96-238](file://src/components/mobile/knowledge/KnowledgeArticle.vue#L96-L238)
- [src/components/mobile/knowledge/KnowledgeProfile.vue:78-146](file://src/components/mobile/knowledge/KnowledgeProfile.vue#L78-L146)
...
KnowledgeFavorite["KnowledgeFavorite表"] --> |article_id| Articles["Articles表"]
KnowledgeReadHistory["KnowledgeReadHistory表"] --> |article_id| Articles
subgraph "衍生表"
Stocks["Stocks表"]
Funds["Funds表"]
...
| idx_knowledge_read_history_read_time | knowledge_read_history | read_time | 阅读历史排序 |
| idx_knowledge_read_history_article_id | knowledge_read_history | article_id | 文章历史查询 |

**更新** 新增了知识管理相关的索引，优化收藏和阅读历史的查询性能。
...
**新增** 系统现在集成了完整的知识管理功能，包括知识收藏表和阅读历史表，支持用户个性化知识管理需求。通过文章分类、收藏、阅读历史等特性，系统为用户提供了丰富的财务知识内容，并通过个性化功能提升了用户体验。知识管理功能与财务功能紧密结合，为用户提供从理论学习到实践应用的完整支持。
