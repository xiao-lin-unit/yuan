...
**更新** 本版本反映了数据库架构的重大变更，从混合模式迁移到单一模式，简化了初始化逻辑，并引入了ended字段支持历史资产追踪。
...
**章节来源**
- [src/database/index.js:1-864](file://src/database/index.js#L1-L864)
...
- [src/components/mobile/asset/AddFundPage.vue:1-322](file://src/components/mobile/asset/AddFundPage.vue#L1-L322)
...
**更新** 数据库管理器现在采用单一模式，不再支持复杂的迁移处理，简化了初始化逻辑。
...
**更新** 初始化逻辑已简化，移除了复杂的迁移处理，专注于基础表结构的创建。
...
**章节来源**
- [src/database/index.js:21-864](file://src/database/index.js#L21-L864)
...
- 索引优化
  - 为高频查询字段建立索引（账户类型、是否流动资金、交易账户外键、时间戳、分类类型等）。
- 查询优化
...
## 结论
该数据库管理方案通过统一的抽象层实现了 Capacitor SQLite 与 SQL.js 的单一模式支持，既满足移动端原生性能，又兼顾 Web/桌面跨平台需求。配合事务、批处理、查询缓存与索引优化，能够有效提升性能与可靠性。新的ended字段设计为资产历史追踪提供了完整支持，简化了数据库初始化逻辑，移除了复杂的迁移处理，使系统更加稳定和易于维护。
...
**章节来源**
- [src/database/index.js:433-688](file://src/database/index.js#L433-L688)
