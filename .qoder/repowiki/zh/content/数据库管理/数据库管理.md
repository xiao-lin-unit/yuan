...
**更新** 本版本反映了数据库架构的重大变更，从混合模式迁移到单一模式，新增了沙盘推演功能，包括沙盘历史表和结果表，支持14种财务推演场景，简化了初始化逻辑，并引入了更完善的事务处理机制。
...
**章节来源**
- [src/database/index.js:1-1017](file://src/database/index.js#L1-L1017)
...
**更新** 数据库管理器现在采用单一模式，不再支持复杂的迁移处理，简化了初始化逻辑，并新增了沙盘推演功能，包括沙盘历史表和结果表的创建与管理。
...
**更新** 初始化逻辑已简化，移除了复杂的迁移处理，专注于基础表结构的创建，并新增了沙盘推演功能所需的沙盘历史表和结果表。
...
- [src/database/index.js:379-408](file://src/database/index.js#L379-L408)
- [src/database/index.js:420-958](file://src/database/index.js#L420-L958)
- [src/database/index.js:826-1017](file://src/database/index.js#L826-L1017)

**章节来源**
- [src/database/index.js:21-1017](file://src/database/index.js#L21-L1017)
...
**更新** 性能考量保持不变，主要针对简化后的初始化逻辑进行优化，并新增了沙盘表的索引优化。
...
- [src/database/index.js:379-408](file://src/database/index.js#L379-L408)
- [src/database/index.js:420-958](file://src/database/index.js#L420-L958)
- [src/database/index.js:839-1017](file://src/database/index.js#L839-L1017)
...
**更新** 故障排查指南保持不变，适用于简化的数据库架构，并新增了沙盘推演相关的问题排查。
...
## 结论
该数据库管理方案通过统一的抽象层实现了 Capacitor SQLite 与 SQL.js 的单一模式支持，既满足移动端原生性能，又兼顾 Web/桌面跨平台需求。配合事务、批处理、查询缓存与索引优化，能够有效提升性能与可靠性。新增的沙盘推演功能进一步增强了财务应用的数据管理能力，包括沙盘历史表和结果表的设计与实现，支持14种财务压力测试场景，简化了数据库初始化逻辑，移除了复杂的迁移处理，使系统更加稳定和易于维护。
...
- 资产表（assets）：主键 id，类型/名称，金额，账户外键，周期，周期数量，收益日期，下一个收益日期，计算类型，每期收益金额，年收益率率，ended状态，时间戳。
...
**章节来源**
- [src/database/index.js:433-825](file://src/database/index.js#L433-L825)
...
**更新** 新增了沙盘功能的最佳实践，包括数据安全和软删除机制的安全建议。
