...
## 更新摘要
**所做更改**
- 新增了沙盘推演功能，包括沙盘历史表（sandbox_history）和沙盘结果表（sandbox_result）
- 新增了沙盘相关的索引优化：idx_sandbox_history_scene_type、idx_sandbox_history_is_deleted、idx_sandbox_result_history_id
- 新增了沙盘服务的完整实现，支持14种财务推演场景
- 新增了沙盘相关的组件：FinancialSandbox、SandboxHistory、SandboxResultDetail、SandboxSimulationPage
- 更新了数据库初始化逻辑，包含沙盘表的创建和索引
- 新增了沙盘推演的CRUD操作和事务处理
- 增强了平台检测机制，改进了连接管理
- 优化了事务处理机制，提升了性能
- 实现了索引优化，提高了查询性能
...
**更新** 本版本反映了数据库架构的重大改进，从混合模式迁移到单一模式，新增了沙盘推演功能，包括沙盘历史表和结果表，支持14种财务推演场景，简化了初始化逻辑，并引入了更完善的事务处理机制和平台检测优化。
...
- 沙盘功能：新增独立的沙盘推演模块，提供14种财务压力测试场景。
...
UI_FinancialSandbox["FinancialSandbox.vue"]
UI_SandboxHistory["SandboxHistory.vue"]
UI_SandboxResultDetail["SandboxResultDetail.vue"]
UI_SandboxSimulation["SandboxSimulationPage.vue"]
end
subgraph "业务层"
Store_Account["stores/account.ts"]
Svc_Category["services/categoryService.ts"]
Svc_Asset["services/asset/assetService.ts"]
Svc_Sandbox["services/sandbox/sandboxService.ts"]
end
subgraph "数据层"
...
UI_FinancialSandbox --> Svc_Sandbox
UI_SandboxHistory --> Svc_Sandbox
UI_SandboxResultDetail --> Svc_Sandbox
UI_SandboxSimulation --> Svc_Sandbox
...
- [src/components/mobile/sandbox/SandboxHistory.vue:37-92](file://src/components/mobile/sandbox/SandboxHistory.vue#L37-L92)
...
- [src/services/asset/assetService.ts:1-165](file://src/services/asset/assetService.ts#L1-L165)
- [src/services/sandbox/sandboxService.ts:1-730](file://src/services/sandbox/sandboxService.ts#L1-L730)
...
- 沙盘服务：提供14种财务推演场景，支持参数化推演、结果存储与历史管理。

**更新** 数据库管理器现在采用单一模式，不再支持复杂的迁移处理，简化了初始化逻辑，并新增了沙盘推演功能，包括沙盘历史表和结果表的创建与管理。平台检测机制得到显著改进，连接管理更加稳健。
...
**更新** 初始化逻辑已简化，移除了复杂的迁移处理，专注于基础表结构的创建，并新增了沙盘推演功能所需的沙盘历史表和结果表。平台检测机制得到显著改进，连接管理更加稳健。
...
- [src/database/index.js:379-408](file://src/database/index.js#L379-L408)
- [src/database/index.js:420-1004](file://src/database/index.js#L420-L1004)
- [src/database/index.js:826-1049](file://src/database/index.js#L826-L1049)

**章节来源**
- [src/database/index.js:21-1049](file://src/database/index.js#L21-L1049)
...
### 资产服务（新增收益日期功能和计算类型）
- 功能：通用资产的增删改查、收益日期计算、活跃资产查询、计算类型支持。
- 收益日期计算：支持年和月两种周期，自动计算下一个收益日期。
- 计算类型：支持"按金额计算"和"按年收益率计算"两种模式。
- 每期收益金额：支持固定金额的收益计算。
- 年收益率率：支持基于年化收益率的收益计算。

**更新** 新增了计算类型支持，包括按金额计算和按年收益率计算两种模式，以及相应的收益金额和年收益率字段。
...
### 资产类型定义（新增收益日期字段和计算类型）
- 功能：定义资产的基础接口和输入接口。
- 新增字段：income_date（收益日期）、next_income_date（下一个收益日期）、calculation_type（计算类型）、income_amount（每期收益金额）、annual_yield_rate（年收益率率）。

**更新** 新增了收益日期相关的类型定义和计算类型支持，包括按金额计算和按年收益率计算两种模式。

**章节来源**
- [src/types/asset/asset.ts:1-31](file://src/types/asset/asset.ts#L1-L31)

### 添加资产页面（新增收益日期输入和计算类型）
- 功能：支持年和月两种周期的收益日期输入，支持计算类型选择。
- 年周期：选择月份和日期，格式为MM-DD。
- 月周期：选择日期，格式为DD。
- 计算类型：支持"按金额计算"和"按年收益率计算"两种模式。
- 资产类型验证：公积金资产必须关联公积金账户。

**更新** 新增了计算类型输入功能，支持按金额计算和按年收益率计算两种模式的选择。
...
### 沙盘推演功能（新增）
- 功能：提供14种财务压力测试场景，支持参数化推演、结果存储与历史管理。
- 场景类型：包括失业维持时长、债务利率上涨、股票/基金下跌、每月多存、提前还清负债等。
- 数据存储：使用sandbox_history表存储推演基础记录，使用sandbox_result表存储详细结果。
- 事务处理：推演结果使用事务保证历史记录和结果的一致性。

**更新** 新增了完整的沙盘推演功能，包括沙盘历史表和结果表的创建、索引优化、CRUD操作和事务处理。

```mermaid
flowchart TD
Start(["进入沙盘推演"]) --> LoadData["加载用户真实财务数据"]
LoadData --> Compute["执行推演计算"]
Compute --> SaveHistory["保存推演历史记录"]
SaveHistory --> SaveResult["保存推演结果"]
SaveResult --> Success["返回推演ID"]
```

**图表来源**
- [src/services/sandbox/sandboxService.ts:279-696](file://src/services/sandbox/sandboxService.ts#L279-L696)
- [src/services/sandbox/sandboxService.ts:698-721](file://src/services/sandbox/sandboxService.ts#L698-L721)

**章节来源**
- [src/services/sandbox/sandboxService.ts:1-730](file://src/services/sandbox/sandboxService.ts#L1-L730)
- [src/components/mobile/sandbox/FinancialSandbox.vue:1-121](file://src/components/mobile/sandbox/FinancialSandbox.vue#L1-L121)
- [src/components/mobile/sandbox/SandboxHistory.vue:1-92](file://src/components/mobile/sandbox/SandboxHistory.vue#L1-L92)
- [src/components/mobile/sandbox/SandboxResultDetail.vue:1-323](file://src/components/mobile/sandbox/SandboxResultDetail.vue#L1-L323)
- [src/components/mobile/sandbox/SandboxSimulationPage.vue:1-154](file://src/components/mobile/sandbox/SandboxSimulationPage.vue#L1-L154)

### 沙盘服务（新增）
- 功能：提供沙盘推演的核心逻辑，包括场景定义、参数验证、财务数据加载、推演计算、结果存储。
- 场景定义：包含14种预设场景，每种场景都有特定的参数和计算逻辑。
- 数据加载：从数据库实时读取用户的真实财务数据，包括资产、负债、收支等。
- 推演计算：根据场景类型和用户参数执行相应的财务推演算法。
- 结果存储：使用事务保证历史记录和结果数据的一致性。

**更新** 新增了完整的沙盘服务实现，支持14种财务推演场景，包括参数验证、数据加载、计算逻辑和结果存储。

**章节来源**
- [src/services/sandbox/sandboxService.ts:1-730](file://src/services/sandbox/sandboxService.ts#L1-L730)

### 沙盘组件（新增）
- FinancialSandbox：沙盘主界面，展示14种推演场景，支持场景选择和导航。
- SandboxSimulation：场景参数输入界面，支持不同类型的参数输入（开关、数字、选择框等）。
- SandboxHistory：推演历史列表界面，支持历史记录查看、详情导航、软删除。
- SandboxResultDetail：推演结果详情界面，支持图表展示、参数解析、结论展示。

**更新** 新增了完整的沙盘组件体系，包括场景展示、参数输入、历史管理和结果展示。
...
- 批处理与事务
  ...
  - 事务：余额调整、转账、沙盘推演等关键流程使用事务，保证原子性与一致性。
- Web 端持久化
  - SQL.js 模式下采用延迟持久化与节流，避免频繁写入 localStorage。
- 索引优化
  - 为高频查询字段建立索引（账户类型、是否流动资金、交易账户外键、时间戳、分类类型、账户流水账户外键、资产表相关索引、沙盘表相关索引等）。
- 查询优化
...
**更新** 性能考量保持不变，主要针对简化后的初始化逻辑进行优化，并新增了沙盘表的索引优化。平台检测机制得到改进，连接管理更加高效。
...
- [src/database/index.js:379-408](file://src/database/index.js#L379-L408)
- [src/database/index.js:420-1004](file://src/database/index.js#L420-L1004)
- [src/database/index.js:839-1049](file://src/database/index.js#L839-L1049)
...
- 计算类型问题
  - 检查assets表的calculation_type、income_amount、annual_yield_rate字段是否正确更新。
  - 确认calculatePerAssetIncome函数的逻辑是否正确。
- 沙盘推演问题
  - 检查sandbox_history和sandbox_result表的结构是否正确。
  - 确认沙盘推演的事务处理是否正常执行。
  - 验证沙盘场景参数的输入和验证逻辑。
- 平台检测问题
  - 检查Capacitor.isNativePlatform()的返回值是否正确。
  - 确认连接一致性检查和连接状态验证逻辑。

**更新** 故障排查指南保持不变，适用于简化的数据库架构，并新增了沙盘推演相关的问题排查。平台检测机制得到改进，连接管理更加稳健。
...
## 结论
该数据库管理方案通过统一的抽象层实现了 Capacitor SQLite 与 SQL.js 的单一模式支持，既满足移动端原生性能，又兼顾 Web/桌面跨平台需求。配合事务、批处理、查询缓存与索引优化，能够有效提升性能与可靠性。新增的沙盘推演功能进一步增强了财务应用的数据管理能力，包括沙盘历史表和结果表的设计与实现，支持14种财务压力测试场景，简化了数据库初始化逻辑，移除了复杂的迁移处理，使系统更加稳定和易于维护。平台检测机制得到显著改进，连接管理更加稳健，事务处理机制优化提升了整体性能。
...
- 沙盘历史表（sandbox_history）：主键 id，场景类型，场景名称，用户标题，推演时间，参数，结果描述，创建时间，软删除标识，备注。
- 沙盘结果表（sandbox_result）：主键 id，历史记录外键，净资产，净资产变动，月现金流，现金流变动，负债压力等级，负债压力描述，可维持月数，节省利息，月供变动，总利息变动，X轴数据，净资产曲线，现金流曲线，负债曲线，结论，创建时间。

**更新** 新增了沙盘推演功能相关的数据模型，包括沙盘历史表和结果表，用于存储推演记录和结果数据。
...
- 分类：type
- 沙盘历史：scene_type、is_deleted
- 沙盘结果：history_id

**章节来源**
- [src/database/index.js:433-846](file://src/database/index.js#L433-L846)
...
- 计算类型：在资产操作中支持按金额计算和按年收益率计算两种模式。
- 沙盘推演：使用事务保证历史记录和结果数据的一致性，支持14种财务推演场景。

**更新** 新增了沙盘推演的使用模式，强调事务处理和14种场景的支持。
...
- 沙盘功能：新增沙盘推演功能，包含历史表和结果表的创建与索引优化。

**更新** 移除了复杂的迁移处理说明，反映了简化的数据库架构，并新增了沙盘功能的迁移说明。

**章节来源**
- [src/database/index.js:420-1004](file://src/database/index.js#L420-L1004)
- [src/database/index.js:759-825](file://src/database/index.js#L759-L825)
- [update.js:1-715](file://update.js#L1-L715)
...
- 计算类型管理：正确选择和使用计算类型，确保收益计算的准确性。
- 沙盘数据安全：沙盘推演使用本地计算和存储，不上云、不发起网络请求，保障用户财务数据安全。
- 软删除机制：沙盘历史记录使用软删除，支持数据恢复和审计追踪。
- 平台检测安全：确保Capacitor.isNativePlatform()的正确性，避免平台误判导致的安全问题。

**更新** 新增了沙盘功能的最佳实践，包括数据安全和软删除机制的安全建议。平台检测机制得到改进，增加了平台检测安全的考虑。
...
- [src/database/index.js:379-408](file://src/database/index.js#L379-L408)
- [src/services/asset/assetService.ts:12-43](file://src/services/asset/assetService.ts#L12-L43)
- [src/services/sandbox/sandboxService.ts:178-180](file://src/services/sandbox/sandboxService.ts#L178-L180)
- [src/components/mobile/asset/AddFundPage.vue:213-240](file://src/components/mobile/asset/AddFundPage.vue#L213-L240)
...
- 计算类型扩展：支持更多计算类型的收益计算，如按月收益率、按日收益率等。
- 自动迁移：使用update.js脚本进行数据库架构更新，确保数据一致性。
- 沙盘功能扩展：基于现有的沙盘表结构，可以添加新的推演场景或优化现有场景的计算逻辑。
- 平台检测扩展：可以进一步增强平台检测机制，支持更多平台类型。

**更新** 扩展指导反映了简化的数据库架构和沙盘功能的扩展可能性。平台检测机制得到改进，可以进一步增强平台检测能力。
...
- [src/components/mobile/DatabaseViewer.vue:100-237](file://src/components/mobile/DatabaseViewer.vue#L100-L237)
- [update.js:1-715](file://update.js#L1-L715)
- [src/services/sandbox/sandboxService.ts:57-155](file://src/services/sandbox/sandboxService.ts#L57-L155)