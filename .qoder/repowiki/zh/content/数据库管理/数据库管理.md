...
- 新增了assets表的calculation_type、income_amount、annual_yield_rate字段，支持新的计算功能
- 更新了资产服务的收益日期计算逻辑，支持年和月两种周期
- 新增了迁移脚本update.js，自动更新数据库架构和相关组件
- 更新了资产类型定义，支持新的收益日期字段和计算类型
...
**更新** 本版本反映了数据库架构的重大变更，从混合模式迁移到单一模式，新增了assets表的收益日期管理功能和计算类型支持，简化了初始化逻辑，并引入了更完善的事务处理机制。
...
**更新** 数据库管理器现在采用单一模式，不再支持复杂的迁移处理，简化了初始化逻辑，并新增了assets表的收益日期管理功能和计算类型支持。
...
**更新** 初始化逻辑已简化，移除了复杂的迁移处理，专注于基础表结构的创建，并新增了assets表的收益日期字段和计算类型支持。
...
**更新** 故障排查指南保持不变，适用于简化的数据库架构，并新增了收益日期和计算类型相关的问题排查。
...
## 结论
该数据库管理方案通过统一的抽象层实现了 Capacitor SQLite 与 SQL.js 的单一模式支持，既满足移动端原生性能，又兼顾 Web/桌面跨平台需求。配合事务、批处理、查询缓存与索引优化，能够有效提升性能与可靠性。新增的assets表收益日期管理功能和计算类型支持进一步增强了财务应用的数据管理能力，简化了数据库初始化逻辑，移除了复杂的迁移处理，使系统更加稳定和易于维护。
...
**更新** 新增了assets表的收益日期字段和计算类型说明，用于资产收益日期管理和计算功能。
...
**更新** 新增了计算类型和收益日期的使用模式，强调自动计算和更新。
...
**更新** 新增了资产生命周期管理和审计追踪的最佳实践，以及收益日期和计算类型管理的安全建议。
...
**更新** 扩展指导反映了简化的数据库架构和account_transactions表的应用，以及新增的收益日期管理和计算类型功能。
