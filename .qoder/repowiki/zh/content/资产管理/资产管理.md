...
## 更新摘要
**变更内容**
- **新增灵活计算类型系统**：支持按金额计算和按年收益率计算两种模式，增强资产管理和详情页面的计算功能
- **新增资产收益计算函数**：`calculatePerAssetIncome` 函数支持不同计算模式的收益金额计算
- **数据库字段扩展**：新增 `calculation_type`、`income_amount`、`annual_yield_rate` 字段支持灵活计算
- **条件账户扣款功能增强**：资产购买、基金购买、股票购买现在都支持deductFromAccount参数，允许用户选择是否从关联账户自动扣款
- **资产详情页面样式重构**：从flex-wrap布局改为space-around布局，提升视觉一致性
- **新增FloatingActionMenu浮动操作菜单组件**：在资产管理组件中集成，增强用户体验
- **资产详情页面新增悬浮操作按钮**：集成FloatingActionMenu组件，提供更好的操作体验
- **资产管理组件新增浮动菜单**：提供新增资产的快捷入口
...
**更新** 本次更新重点反映了应用变更：新增通用资产服务层架构，包括 assetService.ts、fundService.ts、stockService.ts、liabilityService.ts 等完整服务模块，以及新的计算工具类和类型系统扩展。同时，资产总览组件现已集成StatOverview组件，提供更简洁的统计展示。新增灵活资产计算方法，支持固定金额计算和年收益率计算两种模式。**新增条件账户扣款功能，资产购买、基金购买、股票购买都支持deductFromAccount参数，允许用户选择是否从关联账户自动扣款。** **新增资产详情页面样式重构和FloatingActionMenu浮动操作菜单组件，显著提升用户体验。**
...
ADP["AssetDetailPage.vue<br/>资产详情含计算信息<br/>集成FloatingActionMenu"]
...
- **资产详情页面**：展示资产基本信息、计算类型、收益记录等详细信息，**新增FloatingActionMenu浮动操作菜单组件**。
...
**更新** 新增条件账户扣款功能，资产购买、基金购买、股票购买都支持deductFromAccount参数，允许用户选择是否从关联账户自动扣款。新增FloatingActionMenu浮动操作菜单组件，显著提升资产管理的用户体验。资产详情页面集成该组件，提供更好的操作体验。
...
**更新** 资产详情页面完成样式重构，从flex-wrap布局改为space-around布局，显著提升视觉一致性和用户体验。新增FloatingActionMenu浮动操作菜单组件，提供更好的操作体验。
...
**更新** 新增计算类型和收益信息的展示，增强资产详情的完整性。资产详情页面集成FloatingActionMenu组件，提供更好的操作体验。
...
**更新** 新增deductFromAccount参数的性能考量，分支判断开销极小。
...
**更新** 新增deductFromAccount参数相关的故障排查指导。
