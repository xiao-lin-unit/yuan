...
**更新** 本次更新重点反映了应用变更：新增通用资产服务层架构，包括 assetService.ts、fundService.ts、stockService.ts、liabilityService.ts 等完整服务模块，以及新的计算工具类和类型系统扩展。同时，资产总览组件现已集成StatOverview组件，提供更简洁的统计展示。新增灵活资产计算方法，支持固定金额计算和年收益率计算两种模式。**新增资产详情页面样式重构和FloatingActionMenu浮动操作菜单组件，显著提升用户体验。**
...
**更新** 新增FloatingActionMenu浮动操作菜单组件，显著提升资产管理的用户体验。资产详情页面集成该组件，提供更好的操作体验。
...
**更新** 新增FloatingActionMenu组件的性能考量，组件结构简单，动画效果流畅。
...
**更新** 新增FloatingActionMenu组件和资产详情页面布局相关的故障排查指导。
...
**更新** 新增FloatingActionMenu组件和资产详情页面样式重构的业务场景示例，以及相关的用户体验和界面一致性改进。