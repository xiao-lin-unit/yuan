...
graph TB
subgraph "移动端组件"
AM["AssetManagement.vue<br/>资产总览"]
...
- [BuyFundPage.vue:1-269](file://src/components/mobile/asset/BuyFundPage.vue#L1-L269)
- [SellFundPage.vue:1-303](file://src/components/mobile/asset/SellFundPage.vue#L1-L303)
...
## 核心组件
- 资产总览页：聚合通用资产、股票、基金三类卡片，支持浮动菜单新增入口与详情跳转。
- 资产卡片：统一展示资产标题、主金额与次级金额（如成本价），支持点击回调。
- 新增资产/股票/基金：表单校验、账户过滤、写入数据库与交易记录。
- 基金详情与买卖：持有记录、买入/卖出记录、收益计算、锁定期处理、事务提交。

章节来源
- [AssetManagement.vue:10-46](file://src/components/mobile/asset/AssetManagement.vue#L10-L46)
...
HasData -- 是 --> RenderCards["渲染资产卡片"]
RenderCards --> Click["点击卡片"]
Click --> Detail["跳转详情或执行操作"]
```

图示来源
- [AssetManagement.vue:141-227](file://src/components/mobile/asset/AssetManagement.vue#L141-L227)
- [AssetCard.vue:1-180](file://src/components/mobile/asset/AssetCard.vue#L1-L180)
...
- 卖出：按最早买入优先原则扣减剩余份额，支持部分卖出与锁定期校验，更新确认收益。
...
章节来源
- [FundDetailPage.vue:236-412](file://src/components/mobile/asset/FundDetailPage.vue#L236-L412)
...
SFP->>DB : UPDATE funds SET shares/current_nav/confirmed_profit
DB-->>SFP : 成功
SFP-->>U : 返回资产总览
```
...
章节来源
- [SellFundPage.vue:137-211](file://src/components/mobile/asset/SellFundPage.vue#L137-L211)
...
## 结论
资产管理模块以清晰的组件分层与完善的数据库事务为基础，实现了资产总览、股票与基金的全生命周期管理。通过可扩展的卡片组件与统一的数据层，开发者可在不破坏现有结构的前提下，灵活扩展新资产类型、优化计算策略与增强报表能力。
...
- 业务场景示例
  ...
  - 基金卖出：按锁定期与剩余份额规则扣减，生成卖出记录并更新确认收益。
- 资产变动对账户与财务状况的影响
