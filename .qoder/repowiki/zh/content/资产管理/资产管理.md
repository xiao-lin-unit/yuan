...
**更新** 本次更新重点反映了应用变更：新增通用资产服务层架构，包括 assetService.ts、fundService.ts、stockService.ts、liabilityService.ts 等完整服务模块，以及新的计算工具类和类型系统扩展。同时，资产总览组件现已集成StatOverview组件，提供更简洁的统计展示。新增灵活资产计算方法，支持固定金额计算和年收益率计算两种模式。**新增条件账户扣款功能，资产购买、基金购买、股票购买都支持deductFromAccount参数，允许用户选择是否从关联账户自动扣款。** **新增资产详情页面样式重构和FloatingActionMenu浮动操作菜单组件，显著提升用户体验。** **新增交易记录页面统一样式优化，采用space-around布局和统一间距规范，提升界面一致性。**
...
graph TB
subgraph "移动端组件层"
AM["AssetManagement.vue<br/>资产总览含历史资产切换<br/>集成FloatingActionMenu"]
...
**更新** 新增条件账户扣款功能，资产购买、基金购买、股票购买都支持deductFromAccount参数，允许用户选择是否从关联账户自动扣款。新增FloatingActionMenu浮动操作菜单组件，显著提升资产管理的用户体验。资产详情页面集成该组件，提供更好的操作体验。**新增交易记录页面统一样式优化，采用space-around布局和统一间距规范，提升界面一致性。**
...
**更新** 新增FloatingActionMenu浮动操作菜单组件，提供更好的用户体验。资产总览页面集成该组件，支持多按钮展开菜单模式。
...
- **样式优化性能**：space-around布局和统一间距规范的引入对性能影响微乎其微，主要提升用户体验。
- **布局优化性能**：space-around布局相比flex-wrap布局，在元素均匀分布方面性能相当，但视觉效果更佳。

**更新** 新增deductFromAccount参数的性能考量，分支判断开销极小。**新增样式优化性能考量，space-around布局和统一间距规范对性能影响微乎其微。**
...
**更新** 新增deductFromAccount参数相关的故障排查指导。**新增收益记录样式和交易记录统一样式的故障排查指导。**
