...
资产总览功能是金融应用中的核心模块，负责展示用户的各类资产情况。该功能实现了现代化的服务层架构设计，采用分层架构模式，支持通用资产、股票资产和基金资产的分类展示，并提供了完整的资产价值计算逻辑和用户交互体验。**最新更新**集成了专门的服务层架构，包括资产服务、股票服务、基金服务等模块，提供更流畅的用户体验和更丰富的资产信息展示。
...
- 浮动操作菜单的集成，提供新增资产的快捷入口
- 资产分类的处理逻辑，支持通用资产、股票资产、基金资产
- **新增**：历史资产视图切换功能，支持资产状态过滤
- **新增**：现代化服务层集成，提供更好的数据管理
...
- 主要金额和次要金额的双行显示，支持格式化显示
- **更新**：增强的点击事件处理机制，支持资产详情导航

### 服务层架构

**新增** 专门的服务层模块，提供业务逻辑封装：
- **资产服务**：处理通用资产的CRUD操作和业务逻辑
- **股票服务**：处理股票资产的复杂业务逻辑，包括买卖交易、收益计算
- **基金服务**：处理基金资产的业务逻辑，包括锁定期管理、收益计算

**章节来源**
- [AssetManagement.vue:86-281](file://src/components/mobile/asset/AssetManagement.vue#L86-L281)
- [AssetCard.vue:21-66](file://src/components/mobile/asset/AssetCard.vue#L21-L66)
- [assetService.ts:48-165](file://src/services/asset/assetService.ts#L48-L165)
- [stockService.ts:61-482](file://src/services/asset/stockService.ts#L61-L482)
- [fundService.ts:71-508](file://src/services/asset/fundService.ts#L71-L508)
...
**图表来源**
- [AssetManagement.vue:90-97](file://src/components/mobile/asset/AssetManagement.vue#L90-L97)
...
**图表来源**
- [AssetManagement.vue:321-477](file://src/components/mobile/asset/AssetManagement.vue#L321-L477)

#### 资产价值计算逻辑

系统实现了三种类型的资产价值计算，采用现代化的服务层架构：

| 资产类型 | 计算公式 | 服务层实现 | 字段映射 |
|---------|---------|-----------|---------|
| 通用资产 | `amount` | assetService.ts | 直接使用数据库中的金额字段 |
| 股票资产 | `costPrice × quantity` | stockService.ts | 成本价 × 股数 |
| 基金资产 | `cost_nav × shares` | fundService.ts | 成本净值 × 份额 |
...
**图表来源**
- [AssetManagement.vue:127-152](file://src/components/mobile/asset/AssetManagement.vue#L127-L152)
...
**图表来源**
- [AssetManagement.vue:10-12](file://src/components/mobile/asset/AssetManagement.vue#L10-L12)
...
AssetCard.vue 采用了Vue 3 Composition API 的设计模式，支持现代化的组件开发：
...
- 次要金额：显示为 ¥XX.XX 的格式，使用 `toFixed(2)` 方法
- 自动四舍五入到分位
...
FloatingActionMenu.vue 实现了Material Design风格的浮动操作按钮，支持动态按钮数量：
...
ParallelLoad --> LoadFunds[调用fundService.getAllFunds]
...
**章节来源**
- [AssetManagement.vue:226-243](file://src/components/mobile/asset/AssetManagement.vue#L226-L243)
- [assetService.ts:76-78](file://src/services/asset/assetService.ts#L76-L78)
...
**图表来源**
- [asset.ts:6-30](file://src/types/asset/asset.ts#L6-L30)
...
**图表来源**
- [stock.ts:6-95](file://src/types/asset/stock.ts#L6-L95)
...
**图表来源**
- [fund.ts:6-105](file://src/types/asset/fund.ts#L6-L105)
...
CalculateFundNavCostPrice[计算基金净值成本]
CalculateFundWeightedNavCostPrice[计算基金加权净值成本]
CalculateMonthlyAmount[计算月投资额]
end
subgraph "应用场景"
StockCalc[股票计算]
FundCalc[基金计算]
...
CalculateFundWeightedNavCostPrice --> FundCalc
...
| 基金计算 | calculateFundWeightedNavCostPrice | 基金加权净值成本 | 基金追加购买 |
| 基金计算 | calculateFundNavCostPrice | 基金净值成本 | 基金首次购买 |
...
- [AssetManagement.vue:196-201](file://src/components/mobile/asset/AssetManagement.vue#L196-L201)
...
资产总览功能通过现代化的分层架构设计，为用户提供了优秀的资产管理体验。**最新更新**集成了专门的服务层架构，显著提升了系统的可维护性和扩展性。系统的主要优势包括：
...
7. **业务逻辑封装**：专业的股票、基金、通用资产服务层，支持复杂业务场景
