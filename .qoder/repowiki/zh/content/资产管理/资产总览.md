...
## 更新摘要
**变更内容**
- 新增资产添加页面的账户扣款开关功能，允许用户控制购买时的自动扣款行为
- 基金添加页面集成账户扣款开关，支持购买时自动从关联账户扣款
- 股票添加页面新增账户扣款开关，提供灵活的购买扣款控制
- 服务层架构支持账户扣款参数传递，实现统一的出账处理逻辑
- 更新资产卡片组件以支持资产代码显示和更丰富的数据绑定
- 增强浮动操作菜单组件，提供新增资产的快捷入口
...
资产总览功能是金融应用中的核心模块，负责展示用户的各类资产情况。该功能实现了现代化的服务层架构设计，采用分层架构模式，支持通用资产、股票资产的分类展示，并提供了完整的资产价值计算逻辑和用户交互体验。**最新更新**集成了账户扣款开关功能，允许用户在资产添加过程中灵活控制购买时的自动扣款行为，进一步增强了用户体验和资金管理的灵活性。
...
AAP[AddAssetPage.vue<br/>资产添加页面]
AFP[AddFundPage.vue<br/>基金添加页面]
ASP[AddStockPage.vue<br/>股票添加页面]
end
subgraph "服务层"
AS[assetService.ts<br/>通用资产服务]
FS[fundService.ts<br/>基金资产服务]
...
- [AddAssetPage.vue:1-322](file://src/components/mobile/asset/AddAssetPage.vue#L1-L322)
- [AddFundPage.vue:1-267](file://src/components/mobile/asset/AddFundPage.vue#L1-L267)
- [AddStockPage.vue:1-243](file://src/components/mobile/asset/AddStockPage.vue#L1-L243)
...
**新增** 资产添加页面集成了账户扣款开关功能，提供灵活的资金管理控制：
- **新增**：deduct_from_account 开关，默认启用自动扣款
- **新增**：与服务层的统一扣款参数传递机制
- **新增**：购买时的账户余额检查和扣款处理
- **新增**：支持普通资产、基金、股票的统一扣款控制
...
- [AddAssetPage.vue:32-34](file://src/components/mobile/asset/AddAssetPage.vue#L32-L34)
- [AddFundPage.vue:45-47](file://src/components/mobile/asset/AddFundPage.vue#L45-L47)
- [AddStockPage.vue:34-36](file://src/components/mobile/asset/AddStockPage.vue#L34-L36)
...
FUND_HOLDINGS[fund_holdings表]
FUND_TRANSACTIONS[fund_transactions表]
end
UI --> BL
BL --> AS
BL --> FS
...
- [fundService.ts:6-17](file://src/services/asset/fundService.ts#L6-L17)
...
```mermaid
flowchart TD
Start[用户进入添加页面] --> Form[显示表单字段]
Form --> DeductSwitch[显示账户扣款开关]
DeductSwitch --> DefaultOn[默认启用扣款]
DefaultOn --> UserChoice{用户选择?}
UserChoice --> |开启| AutoDeduct[自动扣款]
UserChoice --> |关闭| ManualPayment[手动支付]
AutoDeduct --> ValidateForm[验证表单]
ManualPayment --> ValidateForm
ValidateForm --> CheckBalance{检查账户余额?}
CheckBalance --> |是| ProcessTransaction[处理交易]
CheckBalance --> |否| ShowInsufficient[显示余额不足]
ProcessTransaction --> Complete[添加完成]
ShowInsufficient --> End[操作结束]
Complete --> End
```
...
所有资产添加页面都支持deduct_from_account参数传递：

| 页面类型 | 参数名称 | 默认值 | 作用 |
|---------|---------|--------|------|
| AddAssetPage | deduct_from_account | true | 普通资产购买时自动扣款 |
| AddFundPage | deduct_from_account | true | 基金购买时自动扣款 |
| AddStockPage | deduct_from_account | true | 股票购买时自动扣款 |
...
UI->>Service : 调用添加方法(deductFromAccount)
Service->>Service : 验证输入参数
Service->>Account : 检查账户余额
Account->>Service : 返回余额检查结果
Service->>DB : 执行数据库事务
DB->>Service : 事务执行成功
Service->>UI : 返回操作结果
```
...
**图表来源**
- [assetService.ts:87](file://src/services/asset/assetService.ts#L87)
- [fundService.ts:78](file://src/services/asset/fundService.ts#L78)
- [stockService.ts:70](file://src/services/asset/stockService.ts#L70)

**章节来源**
- [AddAssetPage.vue:108-122](file://src/components/mobile/asset/AddAssetPage.vue#L108-L122)
- [AddFundPage.vue:66-79](file://src/components/mobile/asset/AddFundPage.vue#L66-L79)
- [AddStockPage.vue:59-68](file://src/components/mobile/asset/AddStockPage.vue#L59-L68)
- [assetService.ts:87-124](file://src/services/asset/assetService.ts#L87-L124)
...
- **数据验证**：在服务层进行数据输入验证
- **统一扣款处理**：支持deductFromAccount参数的统一处理
...
fundService.ts 提供专业的基金资产业务逻辑：

```mermaid
graph TB
subgraph "基金服务功能"
ValidateFundCode[验证基金代码]
CheckFundExists[检查基金是否存在]
GetAvailableAccounts[获取可用账户]
CheckAccountBalance[检查账户余额]
AddFund[添加基金]
BuyFund[买入基金]
...
**图表来源**
- [fundService.ts:77-151](file://src/services/asset/fundService.ts#L77-L151)
...
- [fundService.ts:1-200](file://src/services/asset/fundService.ts#L1-L200)
...
| 基金计算 | calculateTotalCost | (nav × shares) + fee | 基金买入成本 |
| 通用计算 | calculateTotalCost | amount | 普通资产成本 |
...
| 基金管理 | funds, fund_holdings, fund_transactions | SELECT | INSERT, UPDATE | Capacitor SQLite, SQL.js |
...
- **统一扣款处理**：deductFromAccount参数的统一处理，减少重复逻辑
...
**症状**：资产添加时扣款失败或余额不足
**可能原因**：
1. **扣款参数传递失败**：deductFromAccount参数未正确传递
2. **账户余额检查错误**：checkAccountBalance函数异常
3. **数据库事务回滚**：扣款操作被回滚
...
- [AddAssetPage.vue:205-223](file://src/components/mobile/asset/AddAssetPage.vue#L205-L223)
- [AddFundPage.vue:146-155](file://src/components/mobile/asset/AddFundPage.vue#L146-L155)
- [AddStockPage.vue:134-143](file://src/components/mobile/asset/AddStockPage.vue#L134-L143)

## 结论

资产总览功能通过现代化的分层架构设计，为用户提供了优秀的资产管理体验。**最新更新**集成了专门的服务层架构、账户扣款开关功能和新的FloatingActionMenu组件，显著提升了系统的可维护性和扩展性。系统的主要优势包括：
...
7. **业务逻辑封装**：专业的资产、股票、基金服务层，支持复杂业务场景
8. **现代化组件**：集成FloatingActionMenu组件，提供更好的用户交互体验
9. **灵活的资金管理**：账户扣款开关功能，支持用户自定义购买扣款行为
10. **统一的扣款处理**：deductFromAccount参数的统一处理，简化业务逻辑
...
// 股票服务调用
import { getAllStocks, addStock } from '@/services/asset/stockService'
...
// 基金服务调用
import { addFund } from '@/services/asset/fundService'
...
const assetForm = ref({
  ...
  income_amount: 0,
  annual_yield_rate: 0,
  deduct_from_account: true, // 默认启用自动扣款
  period: '',
  period_count: 1,
  income_month: 0,
  income_day: 0
