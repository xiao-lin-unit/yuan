...
const addStock = async () => {
  ...
  // 股票代码格式验证：6位数字
  if (!validateStockCode(stockForm.value.code)) {
    alert('股票代码必须为6位数字')
    return
  }
  ...
  try {
    // 检查是否已存在相同股票（通过股票代码）
    const stockCheck = await checkStockExists(stockForm.value.code)
    ...
    alert(error instanceof Error ? error.message : '新增股票失败，请重试')
  }
}
</script>
